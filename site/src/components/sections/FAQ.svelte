...
<section class="bg-[#000030] mx-0 max-w-none w-full p-0">
	<div class="max-w-screen-2xl mx-auto p-8">
		...
		{#each section.faqs as faq, i}
			<button
				class={`block bg-gray-50 w-full text-left p-3 border border-gray-100 ${i == 0 ? 'rounded-t-md' : ''} ${i == section.faqs.length-1 && selectedIndex != i ? 'rounded-b-md' : ''}`}
				on:click={() => {
					// Close all open FAQs
					if (selectedIndex === i) {
						selectedIndex = null;
	...
	<div class="max-w-screen-2xl mx-auto p-8 flex justify-center">
		<div class="border-white border-2 rounded-xl p-5">
			...
			<span class="flex items-center justify-between gap-4">
				<input class="flex-1 p-4 text-black rounded-md" 
				type="string"
